...
#Read in dataset
hadley_data <- read_csv("./Data/hadley_dataset.csv")
#dataset of rows already tweeted
rows_tweeted <- read_csv("./Data/rows_tweeted.csv")
...
row_select <- function(dataset){
  ...
  write_csv(rows_tweeted, "./Data/rows_tweeted.csv")
  return(row_id)
}
...
tweet_sentence <- function(dataset) {
  ...
  tweet <- paste0("Love reading @HadleyFreeman Guardian articles? Then re-read \'", title, "\' from ", date, " ", url)
  return(tweet)
}
...
#....................................
# tweet it
post_tweet(status = paste0(tweet))
